...
function ChapterInsertDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-3 backdrop-blur-sm sm:items-center">
      <div className="card-elevated max-h-[92dvh] w-full max-w-sm space-y-4 overflow-y-auto p-5">
        <div>
  ...
  return (
    <>
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:flex-row">
        ...
        <aside
          className="relative flex max-h-[34dvh] w-full shrink-0 flex-col border-b border-bg-border bg-bg-secondary lg:max-h-none lg:w-[var(--chapters-width)] lg:border-b-0 lg:border-r"
          style={{ '--chapters-width': `${chaptersWidth}px` } as CSSProperties}
        >
          ...
          <div
            ...
            className="absolute -right-1 top-0 hidden h-full w-2 cursor-col-resize bg-transparent transition-colors hover:bg-accent/25 lg:block"
          />
        </aside>
        ...
        <main className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {activeScene ? (
            <>
              {/* Scene header bar */}
              <div className="flex flex-wrap items-center gap-2 border-b border-bg-border bg-bg-secondary/50 px-4 py-3 shrink-0 sm:gap-3 sm:px-5">
                ...
                <div className="flex w-full items-center gap-2 text-xs text-text-muted sm:ml-auto sm:w-auto">
                  {activeScene.blocks.length} beat{activeScene.blocks.length !== 1 ? 's' : ''}
                  <button
                    className={clsx('btn-ghost ml-auto px-2 py-1 text-xs sm:ml-0', rightPanel === 'scene' && 'bg-accent/15 text-accent')}
