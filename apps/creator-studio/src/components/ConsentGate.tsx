...
import { acceptConsents, fetchMissingCreatorConsents, RequiredConsent } from '@/lib/supabase/consents'
...
export function ConsentGate() {
  ...
  useEffect(() => {
    fetchMissingCreatorConsents().then(items => {
      setMissing(items)
      setLoading(false)
    })
  }, [])
  ...
  const accept = async () => {
    setSaving(true)
    setError(null)
    const ok = await acceptConsents(missing, 'creator_studio_access')
    setSaving(false)
    if (!ok) {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 px-4">
      <section className="card-elevated w-full max-w-lg p-5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-text-primary font-semibold">Review creator terms</h2>
            <p className="text-text-secondary text-sm leading-relaxed mt-1">
              Please accept the current creator, privacy, and AI disclosure documents before continuing in Creator Studio.
            </p>
          </div>
        </div>
